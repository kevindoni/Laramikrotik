...
@section('main-content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Add New Usage Log') }}</h1>
                ...
                <div class="card-body">
                    <form action="{{ route('usage-logs.store') }}" method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="datetime-local" class="form-control" id="session_start" name="session_start" 
                                       value="{{ old('session_start', now()->format('Y-m-d\TH:i')) }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="datetime-local" class="form-control" id="session_end" name="session_end" value="{{ old('session_end') }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="number" class="form-control" id="upload_bytes" name="upload_bytes" 
                                       value="{{ old('upload_bytes', 0) }}" min="0">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="number" class="form-control" id="download_bytes" name="download_bytes" 
                                       value="{{ old('download_bytes', 0) }}" min="0">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="nas_port" name="nas_port" value="{{ old('nas_port') }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" id="terminate_cause" name="terminate_cause">
                                    ...
                                    <option value="user_request" {{ old('terminate_cause') == 'user_request' ? 'selected' : '' }}>{{ __('User Request') }}</option>
                                    <option value="lost_carrier" {{ old('terminate_cause') == 'lost_carrier' ? 'selected' : '' }}>{{ __('Lost Carrier') }}</option>
                                    <option value="lost_service" {{ old('terminate_cause') == 'lost_service' ? 'selected' : '' }}>{{ __('Lost Service') }}</option>
                                    <option value="idle_timeout" {{ old('terminate_cause') == 'idle_timeout' ? 'selected' : '' }}>{{ __('Idle Timeout') }}</option>
                                    <option value="session_timeout" {{ old('terminate_cause') == 'session_timeout' ? 'selected' : '' }}>{{ __('Session Timeout') }}</option>
                                    <option value="admin_reset" {{ old('terminate_cause') == 'admin_reset' ? 'selected' : '' }}>{{ __('Admin Reset') }}</option>
                                    <option value="admin_reboot" {{ old('terminate_cause') == 'admin_reboot' ? 'selected' : '' }}>{{ __('Admin Reboot') }}</option>
                                    <option value="port_error" {{ old('terminate_cause') == 'port_error' ? 'selected' : '' }}>{{ __('Port Error') }}</option>
                                    <option value="nas_error" {{ old('terminate_cause') == 'nas_error' ? 'selected' : '' }}>{{ __('NAS Error') }}</option>
                                    <option value="nas_request" {{ old('terminate_cause') == 'nas_request' ? 'selected' : '' }}>{{ __('NAS Request') }}</option>
                                    <option value="other" {{ old('terminate_cause') == 'other' ? 'selected' : '' }}>{{ __('Other') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <div class="col-sm-9 offset-sm-3">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save fa-sm text-white-50"></i> {{ __('Save Usage Log') }}
                                </button>
